...
$get_params = array(
    ...
    'get_fldr_from_filemanager' => $_GET['get_fldr_from_filemanager']
...
if ($ftp) {
    try {
        $current_folder_path = $config['ftp_base_folder'] . $config['upload_dir'] . $rfm_subfolder . $subdir;
    ...
    $current_folder_path = $config['current_path'] . $rfm_subfolder . $subdir;
...
$logged_user_role_id = 1;
// $get_user_accessible_folder = $CI->db->query("SELECT f.* FROM folders f, folders_control fc WHERE fc.f_id = f.f_id AND fc.role_id = '$logged_user_role_id'")->result();
// foreach ($get_user_accessible_folder as $acc_folder) {
//     array_push($access_folders, trim($acc_folder->folder_name));
// }


$current_files_number = 0;
$current_folders_number = 0;
if ($current_folder_path == "../source/" || $current_folder_path == "base_folder/assets/responsive_filemanager/source/") 
{
    foreach ($files as $k => $file) {
        if ($logged_user_role_id == 1) {
            ...
            if (in_array($file, $access_folders)) {
                if ($ftp) {
                    $date = strtotime($file['day'] . " " . $file['month'] . " " . date('Y') . " " . $file['time']);
                    $size = $file['size'];
                    if ($file['type'] == 'file') {
	...
	<div class="span12">
        ...
        foreach ($files as $file_array) {
            ...
            if($src=='') $src="/";
                ...
                if(strpos($_GET['fldr'], '/') !== false){
                    $exp_arr = explode("/",$_GET['fldr']);
                    $re_exp_arr = array_reverse($exp_arr);
                    $rmv_first_index = array_shift($re_exp_arr);
                    $curr_opened_folder = $re_exp_arr[0];
                } else{
                    $curr_opened_folder = $_GET['fldr'];
                }
                
            ?>
            <?php if ($_GET['get_fldr_from_filemanager'] != $curr_opened_folder) { ?>
                ...
                <a class="folder-link" href="dialog.php?<?php echo $get_params.rawurlencode($src)."&".($callback?'callback='.$callback."&":'').uniqid() ?>">
                    <?php // echo $get_params.rawurlencode($src)."&".($callback?'callback='.$callback."&":'').uniqid();echo "<br>"; ?>
            ...
            <?php }else{ ?>
                    ...
                    <?php if ($current_folder_path == "../source/" || $current_folder_path == "base_folder/assets/responsive_filemanager/source/") { echo ""; } else { ?>
                    <figcaption>
