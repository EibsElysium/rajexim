...
<html lang="en">
<head>
  <title>Lead Pivot Report Export</title>
  <meta charset="utf-8">
  ...
  <table class="table table-bordered" id="pivot_table">
    ...
    <tbody>
      ...
      <tr>
          ...
          <?php $i = 1;  foreach($loop_array_top as $top_name){  ?>
          <td><?php $get_count_of_xy = get_dynamic_pivot_report($left_side, $top_of_table, $left_name['left_column_id'], $top_name['top_column_id'],$lst , $ls, $dtrange, $lst, $p_year, $p_month, $product, $country, $ass_to); echo $get_count_of_xy->lead_count; ${'col_tot$j'} = ${'col_tot$j'} + $get_count_of_xy->lead_count; ${"row_tot$i"} = (int)${"row_tot$i"} + (int)$get_count_of_xy->lead_count; ?></td>
        <?php $i++; } ?>
