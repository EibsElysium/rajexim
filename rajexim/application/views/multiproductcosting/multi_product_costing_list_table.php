                              ...
                              <div class="row">
                                 <div class="col-lg-12">  
                                    <table class="table table-striped- table-bordered table-checkable" id="">
                                       ...
                                       <tbody>
                                          <?php if(count($multi_product_costing_lists_count)>0){$i=0;foreach ($multi_product_costing_list as $qlist){
                                             $pclist = $this->Multiproductcosting_model->get_multi_product_costing_by_id($qlist['mpcid']);

                                             $mpclist = $this->Multiproductcosting_model->get_multi_product_costing_by_parent_id($pclist->parent_costing_id);
                                             
                                             $isapp=0; foreach($mpclist as $plist){
                                          ...
                                          <tr bgcolor="<?php echo $rowcolor;?>">
                                             <td>                                                
                                                <h5 class="text-black" style="margin-bottom: 0px;"><?php echo $qlist['multi_product_costing_no'];?> <?php if($qlist['mpccount']>1){?><span class="m-badge m-badge--info pull-right tooltip-animation" data-toggle="m-tooltip" data-placement="top" title="<?php echo $qlist['mpccount']-1; ?> MPC Revised"><?php echo $qlist['mpccount']-1; ?></span><?php }?></h5>
                                             ...
                                             <td>
                                                ...
                                                <a href="<?php echo base_url(); ?>multiproductcosting/multi_product_costing_view/<?php echo $pclist->multi_product_costing_id;?>" target="_blank"><span class="tooltip-animation" data-toggle="m-tooltip" data-placement="top" title="View"><i class="fa fa-info-circle"></i></span></a>&nbsp;&nbsp;
                                                <?php }?>
...
<div class="row">
 <div class="col-lg-12" >
   <div class="col-lg-2">Showing <?php echo $page + 1; ?> to <?php echo (($page + $perpage) > count($multi_product_costing_lists_count)) ? count($multi_product_costing_lists_count) : $page + $perpage; ?> of <?php echo count($multi_product_costing_lists_count); ?> entries</div>
   <div class="col-lg-7"></div>
   <div class="col-lg-3">
    <?php if (count($multi_product_costing_lists_count) > $perpage) { ?>
    <ul class="pagination">
      ...
      <?php 
        $last_page_count = '';
        $split_list = (int) (count($multi_product_costing_lists_count) / $perpage);
        $remaining_val = count($multi_product_costing_lists_count) % $perpage;
        if ($remaining_val == 0) {
      ...
      <?php }  ?>
      <?php if (($page + $perpage) < count($multi_product_costing_lists_count)){ ?>
