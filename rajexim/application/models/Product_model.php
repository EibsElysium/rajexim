...
class Product_model extends CI_Model
{
    ...
    public function product_list($letter, $industry, $search_val, $page, $perpage)                 
    {
        ...
        if ($search_val != '') {
            $sc = ' AND (idus.industry_name LIKE "%'.$search_val.'%" OR p.product_name LIKE "%'.$search_val.'%")';
            // $data['page'] = $page = '0';
        }
        else {
            $sc = '';
        }
        // echo "SELECT p.*, idus.industry_name FROM products p
        // LEFT JOIN industries  idus ON idus.industry_id = p.industry_id
        // WHERE p.status !=2 $letter_val $industry_val $sc LIMIT $page, $perpage";
        // die();
    ...
    public function product_list_for_items($letter, $industry)                 
    {
    ...
    public function product_list_count($letter, $industry,$search_val)                 
    {
        if($letter != '' && $letter != 'all' )
        {
          $letter_val = " AND p.product_name LIKE '$letter%'";
        ...
        $sql = $this->db->query("SELECT p.*, idus.industry_name FROM products p
        LEFT JOIN industries  idus ON idus.industry_id = p.industry_id
        WHERE p.status !=2 $letter_val $industry_val $sc"); 
        return $sql->result(); 
    }
    public function lead_product_list()
    {
        $result = $this->db->query("SELECT p.* FROM products p WHERE p.status = 0 AND p.for_lead = 1")->result();
        return $result; 
    }
    ...
    {
        $result = common_select_values('p.product_id, p.industry_id, p.product_name, p.status, p.description, p.for_lead, p.for_vendor, (SELECT GROUP_CONCAT(industry_name) FROM industries WHERE FIND_IN_SET(industry_id, p.industry_id)) as industry_name ', 'products p', ' p.status !=2 AND p.product_id = "'.$product_id.'"', 'row');
        return $result;
    }
    // To get product by id
    ...
    public function product_update($data)
    {
        ...
        if($data != '')
        {
            $columns = "industry_id = '".trim($data['industry_id'])."',  product_name = '".trim($data['product_name'])."', description = '".trim($data['description'])."' , for_lead = '".trim($data['for_lead'])."' , for_vendor = '".trim($data['for_vendor'])."', modified_by = '".trim($data['modified_on'])."',  modified_by = '".trim($data['modified_by'])."'";
            $condition = ' product_id = "'.trim($data['product_id']).'"';
            $result = common_update_values($columns, 'products', $condition);
        }
        else
    ...
    {
        $result = common_select_values('pi.*, (select product_name from products where product_id = pi.product_id) as product_name, (select industry_name from industries where industry_id = pi.industry_id) as industry_name, (select container_name from container where container_id = pi.container_id) as container_name', 'product_items pi', ' pi.status != 2', 'result');
        return $result; 
    }
    ...
    public function getSubStageValue($piid)
    {
        $result = $this->db->query("CALL getSubStageValue('".$piid."')")->row();
        save_query_in_log();
        return $result;
    }
    ...
    public function get_display_name_by_piid($piid) {
        $result = $this->db->query("CALL get_display_name_by_piid('" . $piid . "')")->result_array();
        save_query_in_log();
        return $result;
    }
    public function delete_display_name_by_piid($piid) {
        $result = $this->db->query("CALL delete_display_name_by_piid('" . $piid . "')");
        save_query_in_log();
        return $result;
    }
    public function create_display_name($data) {
        $result = $this->db->query("CALL create_display_name('" . $data['product_item_id'] . "','" . $data['display_name'] . "')");
        save_query_in_log();
        return $result;
    }

    public function get_product_unit_by_id($puid)
    {
        $result = $this->db->query("CALL get_product_unit_by_id('".$puid."')")->row();
        save_query_in_log();
        return $result;
    }
}
