...
class User_model extends CI_Model
{
    ...
    public function user_email_save($insert_columns, $table_name, $insert_values)
    {
        $result = common_insert_values($insert_columns, $table_name, $insert_values);
        return $result;
    }
    ...
    public function user_list($search_val,$page,$perpage,$user_type)                 
    {
        if ($user_type != '') {
            $user_type_filt = " u.status = 2";
        }
        else {
            $user_type_filt = " u.status != 2";   
        }
        ...
        if ($search_val != '') {
            $sc = ' AND (r.role_name LIKE "%'.$search_val.'%" OR u.name LIKE "%'.$search_val.'%" OR u.contact_no LIKE "%'.$search_val.'%" OR u.username LIKE "%'.$search_val.'%")';
            // $data['page'] = $page = '0';
        }
        else {
            $sc = '';
        ...
        WHERE $user_type_filt $sc GROUP BY u.user_id ORDER BY u.user_id DESC LIMIT $page, $perpage"); 
        return $sql->result(); 
    }
    public function user_list_count($search_val,$user_type)                 
    {
        if ($user_type != '') {
            $user_type_filt = " u.status = 2";
        }
        ...
        $sql = $this->db->query("SELECT u.* , r.role_name, GROUP_CONCAT(p.product_name) as user_product FROM users u
            ...
            LEFT JOIN user_products up ON up.user_id = u.user_id
            LEFT JOIN products p ON p.product_id = up.product_id
        WHERE $user_type_filt $sc GROUP BY u.user_id ORDER BY u.user_id DESC"); 
        return $sql->result(); 
    }

    public function get_user_list_info()
    {
        $sql = $this->db->query("SELECT u.* , r.role_name, GROUP_CONCAT(p.product_name) as user_product FROM users u
            LEFT JOIN roles r ON r.role_id = u.role_id
            LEFT JOIN user_products up ON up.user_id = u.user_id
            LEFT JOIN products p ON p.product_id = up.product_id
        WHERE u.status !=2 GROUP BY u.user_id ORDER BY u.user_id DESC");
        return $sql->result(); 
    ...
    public function users_emails_by_user_id($user_id)
    {
        $result = $this->db->query("SELECT uoe.* FROM user_owned_emails uoe WHERE uoe.user_id = '$user_id'")->result();
        return $result;   
    }
    ...
    public  function delete_user_product_userid($user_id)
    {
        $result = common_delete_values('user_products', ' user_id = "'.$user_id.'"');
        return $result;
    } 
    // To delete user prodct id
    public  function delete_user_email_user_id($user_id)
    {
        $result = common_delete_values('user_emails', ' user_id = "'.$user_id.'"');
        return $result;
    } 
    public function delete_user_owned_email_user_id($user_id)
    {
        $result = common_delete_values('user_owned_emails', ' user_id = "'.$user_id.'"');
        return $result;
    }
    // To delete user prodct id
    ...
    public function update_users_emails($user_owned_emails_id,$user_ID,$email_id,$c_by,$c_on)
    {
        $query = $this->db->query("UPDATE user_owned_emails uoe SET uoe.user_id='$user_ID',uoe.user_emails='$email_id',uoe.modified_on='$c_on',uoe.modified_by='$c_by' WHERE uoe.user_owned_emails_id = '$user_owned_emails_id'");
        return 1;
    }
    public function remove_user_email_by_id($id)
    {
        $query = $this->db->query("DELETE FROM user_owned_emails WHERE user_owned_emails_id = '$id'");
        return 1;   
    }
    public function get_user_login_history($u_id)
    {
        $query = $this->db->query("SELECT lh.*,SEC_TO_TIME( SUM(time_to_sec(lh.log_diff))) AS total_hours_per_day FROM login_history_details lh WHERE lh.user_id = '$u_id' GROUP BY DATE(lh.login_time)")->result();
        return $query;    
    }
    public function get_user_per_day_login_history($u_id,$date)
    {
        $query = $this->db->query("SELECT lh.* FROM login_history_details lh WHERE DATE(lh.login_time) = '$date' AND lh.user_id = '$u_id'")->result();
        return $query;   
    }
    public function chk_role_has_lead_acc($role_id)
    {
        $query = $this->db->query("SELECT r.* FROM roles_permission r WHERE r.menu_id = 12 AND r.role_id = '$role_id'")->row();
        return $query; 
    }
    public function user_delete($user_id)
    {
        $result = common_delete_values('users', '  user_id = "'.$user_id.'"');
        return 1;
    }
}
